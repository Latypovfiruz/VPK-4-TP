...
| Компонент        | Описание |
|------------------|----------|
| **send_command** | Модуль отправки управляющих команд дрону. Формирует и передаёт команды управления (движение, старт, остановка, режимы работы) через брокер сообщений Mosquitto с использованием MQTT-протокола. Используется веб-интерфейсом и другими сервисами системы. |
...
| **mapper**       | Модуль работы с картографическими данными. Отвечает за отображение маршрута полёта, текущего положения дрона и обработку координат (GPS/ГЛОНАСС). Может использоваться для визуализации данных на карте и анализа полётных заданий. |
...
| **Raspberry Pi Zero** | Встроенный модуль дрона с камерой. Отвечает за захват видео/изображений, выполнение команд, полученных через Mosquitto, и отправку данных (видео, телеметрия) в систему. Может выступать в роли edge-устройства. |
| **app.py (Flask)** | Веб-интерфейс системы, реализованный с использованием фреймворка Flask. Предоставляет пользователю интерфейс для управления дроном, просмотра состояния, отправки команд и взаимодействия с другими компонентами через API и MQTT. |
