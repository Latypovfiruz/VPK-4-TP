...
|-----------------|-----------------------------|---------------------------|--------------------------------------------------------------------------------------------------------|
| send_command    | Недоверенный                | MM                        | Модуль формирует команды управления; ошибки или компрометация могут привести к некорректной миссии дрона, нарушая целостность полета (ЦБ1). |
| emulator        | Недоверенный                | MM                        | Эмулятор может генерировать некорректную телеметрию; используется только для тестирования, но ошибки могут нарушить корректность анализа миссии. |
| mapper          | Доверенный, повышает целостность | SS                    | Обрабатывает и визуализирует координаты и маршрут; ошибки могут исказить данные о положении дрона, влияя на контроль миссии. |
| mosquitto       | Доверенный                  | SS                        | Центральный брокер сообщений; сбой или компрометация может нарушить обмен данными между компонентами, влияя на ЦБ1 и ЦБ2. |
| Raspberry Pi Zero | Недоверенный              | MM                        | Выполняет команды дрона и собирает медиа; сбой или компрометация может привести к некорректной телеметрии или утечке данных. |
| app.py (Flask)  | Недоверенный                | MM                        | Предоставляет Read-only интерфейс; ошибки в приложении могут привести к некорректному отображению данных, но не управлению дроном. |
| monitor         | Доверенный, повышает целостность | SS                    | Контролирует все сообщения, проверяет соответствие политикам безопасности; предотвращает выполнение запрещённых команд и защиту данных, обеспечивая ЦБ1 и ЦБ2. |
